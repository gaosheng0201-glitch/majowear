...
1. 第一版是否只支持女装 / 男装 / 中性服装，还是全品类开放？
2. 图片生成优先支持哪种展示模式：白底设计图、模特图、lookbook、flat lay？
3. 是否需要支持中文和英文 prompt 双语？
4. 面料参数是否允许用户自定义字段？
5. Style DNA 是项目级还是用户全局级？
6. 用户是否需要公开分享作品页？
7. 是否需要记录参考图来源和版权信息？
8. 是否需要与现有 Obsidian / Notion / Milanote 等灵感管理工具打通？
9. 是否优先做 Web App，还是同时考虑 iPad 端？
10. 是否要为后续商业化保留团队协作和权限系统？
