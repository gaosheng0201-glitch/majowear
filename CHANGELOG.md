...
- **Prompt Input @-Mentions (Tokenized Pill Input & Semantic Integration)**: Added `@` auto-complete popup inside the Agent Chat input. Referencing a garment converts it into a high-visibility, interactive Pill Tag block. The input window is upgraded to an auto-expanding `<textarea>` (supporting Shift+Enter for new lines, Enter to submit, and auto-growing up to 5 lines/100px before displaying scrollbars). Added explicit backend semantic comparison rules: the agent is trained to identify when users request comparisons of @-referenced garments, automatically rendering structured side-by-side Markdown comparison tables and professional studio scenario analyses.
